...
export function buildOrdersWhere(
  ...
  // Rider Overview's filter: every parcel this rider has ever handled, on
  // either leg (pickup or delivery) — matches the per-rider totals shown on
  // Rider Management, not just the parcels currently out with them for
  // delivery (that narrower question is deliveryRiderId, above).
  if (query.riderId) {
