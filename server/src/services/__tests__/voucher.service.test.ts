...
const { db } = vi.hoisted(() => ({ db: {
  ...
  voucher_claims: { findUnique: vi.fn(), findMany: vi.fn(), create: vi.fn() },
...
import {
  ...
  listAvailableVouchers, listMyVouchers,
...
function offer(overrides = {}) {
  return { id: 'v1', code: 'MOVE100', title: 'Rs. 100 off', description: 'd',
    ...
    claim_limit: 500, claimed_count: 10, is_active: true, ...overrides };
}
...
describe('voucher discount math', () => {
  ...
  it('explains why a claimed voucher is not usable', () => {
    const shaped = { id: 'v1', code: 'MOVE100', title: 't', description: 'd', discountType: 'fixed',
      ...
      startsAt: future(-1), expiresAt: future(1), claimLimit: 5, claimedCount: 1, isActive: true, campaignId: null };
...
describe('voucher claiming', () => {
  it('pins claims to vendor accounts with approved KYC', async () => {
    ...
    await expect(claimVoucher(vendor, { code: 'MOVE100' })).rejects.toThrow('approved KYC');
    expect(db.voucher_claims.create).not.toHaveBeenCalled();
  });
  it('rejects fully claimed offers and repeat claims', async () => {
    ...
    db.voucher_claims.findUnique.mockResolvedValue({ id: 'existing' });
    await expect(claimVoucher(vendor, { code: 'MOVE100' })).rejects.toThrow('already claimed');
  });
