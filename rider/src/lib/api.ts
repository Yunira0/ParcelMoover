...
export interface Parcel {
  ...
  createdAt: string
  /** When the parcel entered its current status - what "today" means for the queue split. */
...
export const RIDER_TRANSITIONS: Partial<Record<ParcelStatus, ParcelStatus[]>> = {
  ...
  // A failed pickup isn't a dead end - the server lets the assigned rider
  // release it back into the dispatch pool themselves (pickup_ordered)
  // rather than leaving it stuck until an admin intervenes.
  // rider_assigned/sent_for_delivery are the reclaim self-assign exception
  // below - ANY rider may scan a failed attempt and take it back over
  // themselves, not just whoever failed it.
  failed_pickup:     ['pickup_ordered', 'rider_assigned'],
  // A failed delivery only offers the reclaim - no dispatch-release option,
  // so scanning it always re-claims straight to sent_for_delivery.
  failed_delivery:   ['sent_for_delivery'],
  // Rider self-assign: scanning one of these claims it straight to the
  // scanning rider (writing pickup_rider_id/delivery_rider_id), skipping the
  // admin/vendor rider-picker (see the matching claim exception in
  // _updateParcelStatusImpl). Only reachable via scan - getOrderByTrackingId
  // exposes these to any rider precisely so the lookup succeeds, since
  // they're not (yet, or no longer) in that rider's own queue to list.
...
// right now). A picked_up parcel has no rider action to take at that moment
// but is still theirs until they hand it over at the hub.
//
// 'dispatched' is deliberately NOT here. It means the parcel has left the
// origin hub, which is where the pickup rider's job ends - and the status
// immediately before it ('arrived') isn't in this queue either, so including
// it made a parcel vanish at hub arrival and then reappear days later, often
// carried by a 3PL the rider has nothing to do with.
export const PENDING_QUEUE_STATUSES: ParcelStatus[] = [
  'rider_assigned', 'picked_up', 'sent_for_delivery',
]
...
export interface RiderParcelsPageResult {
  data: Parcel[]
  hasNextPage: boolean
  /** Opaque - pass straight back in as `cursor` to fetch the next page. */
  nextCursor: string | null
}

// Real (keyset) pagination for lists that can genuinely grow without bound -
// a rider's lifetime order history, unlike the actionable queue, has no
// natural cap. Passing `cursor` walks forward from the previous page's
// nextCursor; omitting it starts from the top.
export async function getRiderParcelsPage(
  statuses: ParcelStatus[],
  opts: { cursor?: string | null; pageSize?: number } = {},
): Promise<RiderParcelsPageResult> {
  const { data } = await api.get<{
    ...
    params: {
      ...
      ...(opts.cursor ? { cursor: opts.cursor, dir: 'next' } : {}),
    },
  })
  return {
    data: data.data ?? [],
...
// Lightweight per-status-group totals (tab badge counts) - independent of
// whatever page of history has actually been loaded, so a lane's count stays
// correct even once its list is paginated.
export async function getRiderStatusCounts(
  groups: Record<string, ParcelStatus[]>,
...
export async function changeRiderPassword(currentPassword: string, newPassword: string): Promise<void> {
  // A password change revokes every existing token (including the one this
  // very request just authenticated with) and the server reissues a fresh
  // one specifically so a Bearer-only client can keep the session alive -
  // without storing it here, the next request 401s and the rider is bounced
  // straight back to the login screen right after "successfully" changing
  // their password.
